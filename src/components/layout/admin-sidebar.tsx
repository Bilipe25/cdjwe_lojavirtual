                        ...
                        {collapsed ? (
                            <DropdownMenu>
                                    ...
                                    <TooltipTrigger render={(
                                        <DropdownMenuTrigger render={(
                                            <Button
                                                ...
                                                className={cn(
                                                    'w-full justify-center px-2 gap-3 text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent',
                                                    isFiscalDocumentsActive && 'bg-sidebar-accent text-sidebar-primary font-medium'
                                                )}
                                            >
                                ...
                                <DropdownMenuContent side="right" sideOffset={16} align="start" className="w-56">
                                    ...
                                    {fiscalDocumentsNavItems.map((item) => {
                                        const isActive = pathname.startsWith(item.href)
                                        return (
                                            <DropdownMenuItem key={item.href} render={(
                                                <Link href={item.href} className={cn(
                            ...
                            <div className="space-y-1">
                                <Button
                                    variant="ghost"
                                    onClick={() => setFiscalDocumentsOpen(!fiscalDocumentsOpen)}
                                    className={cn(
                                        'w-full justify-start gap-3 text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent',
                                        isFiscalDocumentsActive && !fiscalDocumentsOpen && 'text-sidebar-foreground font-medium'
                                    )}
                                >
                                    <ScrollText className="h-5 w-5 shrink-0" />
                                    <span className="flex-1 text-left truncate">Documentos fiscais</span>
                                    <ChevronDown className={cn('h-4 w-4 shrink-0 transition-transform duration-200', effectiveFiscalDocumentsOpen && 'rotate-180')} />
                                </Button>
                                <div className={cn(
                                    'grid transition-all duration-200 ease-in-out',
                                    effectiveFiscalDocumentsOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'
                                )}>
                                    <div className="overflow-hidden">
                                        <div className="pl-9 pr-2 py-1 space-y-1 relative before:absolute before:left-5 before:top-2 before:bottom-2 before:w-px before:bg-sidebar-border">
                                            {fiscalDocumentsNavItems.map((item) => {
