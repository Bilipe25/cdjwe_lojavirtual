    ...
    content: [
      ...
      // Customer Info
      {
        table: {
          ...
          body: [
            ...
            [{
                stack: [
                    { text: 'Endereço:', fontSize: 8, color: '#64748b' },
                    { text: [
                        order.store?.address, 
                        order.store?.city, 
                        order.store?.state,
                        order.store?.zip_code ? `CEP: ${order.store.zip_code}` : ''
                    ].filter(Boolean).join(', ') || 'Endereço não informado', fontSize: 9, bold: true }
                ],
                margin: [10, 5, 10, 10],
                border: [false, false, false, false]
            }],
      ...
      { text: 'ITENS DO PEDIDO', fontSize: 10, bold: true, color: '#0f172a', margin: [0, 0, 0, 10] },
      {
        table: {
          ...
          body: [
            [
              { text: 'PRODUTO / DESCRIÇÃO', bold: true, fontSize: 9, fillColor: '#f8fafc', border: [false, true, false, true], margin: [0, 8, 0, 8] },
            ...
            ...items.map(item => [
              ...
              { text: `R$ ${item.unit_price.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, fontSize: 10, alignment: 'right', margin: [0, 8, 0, 8], border: [false, false, false, true] },
              { text: `R$ ${item.subtotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, fontSize: 10, bold: true, alignment: 'right', margin: [0, 8, 0, 8], border: [false, false, false, true] },
            ] as any)
          ]
        },
        layout: 'noBorders',
        ...
        columns: [
          {
            width: '*',
            stack: [
              { text: 'CONDIÇÃO DE PAGAMENTO', fontSize: 9, bold: true, color: '#0f172a', margin: [0, 0, 0, 6] },
              { 
              ...
              order.notes ? { 
                stack: [
                  { text: 'OBSERVAÇÕES', fontSize: 9, bold: true, color: '#0f172a', margin: [0, 20, 0, 6] },
            ...
            stack: [
              {
                columns: [
                  ...
                  { text: `R$ ${order.subtotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, fontSize: 10, alignment: 'right', bold: true }
                ],
                margin: [0, 0, 0, 6]
              },
              {
                columns: [
                  ...
                  { text: `- R$ ${order.discount_amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, fontSize: 10, alignment: 'right', color: '#16a34a', bold: true }
                ],
                margin: [0, 0, 0, 10]
              },
              { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 170, y2: 0, lineWidth: 1, lineColor: '#e2e8f0' }], margin: [0, 0, 0, 10] },
              {
                columns: [
                  ...
                  { text: `R$ ${order.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, fontSize: 16, bold: true, alignment: 'right', color: '#c2410c' }
                ]
              }
            ]
          }
