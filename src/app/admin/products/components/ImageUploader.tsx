    ...
    return (
        <div className="space-y-4">
                    ...
                    <div key={img.id} className={`relative aspect-square rounded-lg border-2 overflow-hidden group ${primaryImageId === img.id ? 'border-bronze' : 'border-border'}`}>
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                            {primaryImageId !== img.id && (
                                <Button type="button" size="sm" variant="secondary" className="h-7 text-[10px] w-20" onClick={() => onSetPrimary(img.id)}>Capa</Button>
                            )}
                            <Button type="button" size="icon" variant="destructive" className="h-7 w-7" onClick={() => onRemoveExisting(img.id)}><Trash2 className="h-3 w-3" /></Button>
                        </div>
                    ...
                    <div key={`new_${i}`} className={`relative aspect-square rounded-lg border-2 overflow-hidden group ${primaryImageId === `new_${i}` ? 'border-bronze' : 'border-border'}`}>
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                            {primaryImageId !== `new_${i}` && (
                                <Button type="button" size="sm" variant="secondary" className="h-7 text-[10px] w-20" onClick={() => onSetPrimary(`new_${i}`)}>Capa</Button>
                            )}
                            <Button type="button" size="icon" variant="destructive" className="h-7 w-7" onClick={() => onRemoveNew(i)}><X className="h-3 w-3" /></Button>
                        </div>
            ...
            <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3 text-sm text-blue-800 flex items-start mt-4">
                ...
                <p className="text-xs">
                    Faça o upload de imagens em formato 1:1 (quadradas). Máximo 5 fotos. Limite de 5MB por foto.
                </p>
            </div>
        </div>
    );
