...
interface NcmQueryState {
    versionId: string | null
    search: string
    includeStructuralRows: boolean
    page: number
    pageSize: number
    sortBy: FiscalNcmSortBy
    sortOrder: SortOrder
}
...
function getSortLabel(sortBy: FiscalNcmSortBy) {
    switch (sortBy) {
        case 'code':
            return 'Código'
        case 'description':
...
function NcmGridSkeleton() {
    return (
        <div className="space-y-2 px-4 py-4">
            {Array.from({ length: 8 }).map((_, index) => (
                <div key={index} className="grid grid-cols-[120px_1.6fr_1.8fr_120px_120px_140px_100px_80px] gap-3">
...
export function FiscalNcmTablePage({ initialResult, initialVersions }: FiscalNcmTablePageProps) {
    ...
    const queryRef = useRef<NcmQueryState>({
        ...
        sortBy: (initialResult.sortBy as FiscalNcmSortBy) || 'code',
    ...
    const refreshVersions = async () => {
        ...
        const response = await listFiscalReferenceVersionsAction('ncm')
        if (!response.success || !response.data) {
            toast.error(response.error || 'Não foi possível atualizar as versões da base NCM.')
    ...
    const loadEntries = async (overrides: Partial<NcmQueryState> = {}) => {
        const nextQuery: NcmQueryState = {
            ...queryRef.current,
            ...overrides,
        }
        ...
        if (!response.success || !response.data) {
            toast.error(response.error || 'Não foi possível carregar a base NCM.')
    ...
    const handleSort = (field: FiscalNcmSortBy) => {
    ...
    const handleActivateVersion = async () => {
        ...
        toast.success('Versão da base NCM ativada com sucesso.')
        ...
        <div className="space-y-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <div className="space-y-2">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold font-heading text-gradient-navy">Base NCM</h1>
                        <p className="mt-1 max-w-3xl text-sm text-slate-600">
                            Consulte a base versionada da NCM com paginação, rastreabilidade normativa e leitura operacional para revisão fiscal.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <Button asChild className="gradient-navy border-0 text-white">
                        <Link href="/admin/fiscal-bases/imports/new?type=ncm">
            ...
            {showNoVersions ? (
                <div className="rounded-3xl border border-dashed bg-white p-10 text-center shadow-sm">
                    ...
                    <h2 className="text-lg font-semibold text-navy">Nenhuma versão NCM importada ainda</h2>
                    <p className="mx-auto mt-2 max-w-2xl text-sm text-slate-600">
                        Importe a primeira versão da NCM para habilitar consulta, auditoria de vigência e vínculo inteligente com perfis tributários.
                    </p>
                    ...
                    <div className="rounded-3xl border bg-white shadow-sm">
                        <div className="sticky top-0 z-10 rounded-t-3xl border-b bg-white/95 px-5 py-4 backdrop-blur">
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="grid gap-3 xl:grid-cols-[minmax(260px,1.2fr)_210px_210px_180px_auto_auto]">
                                    <div className="relative">
                                        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                        <Input
                                            value={searchInput}
                                            onChange={(event) => setSearchInput(event.target.value)}
                                            placeholder="Buscar por código, descrição ou descrição completa"
                                    ...
                                    <label className="flex min-h-11 items-center gap-3 rounded-2xl border border-slate-200 px-4 text-sm text-slate-700">
                                        <Switch
                                        ...
                                        />
                                        Mostrar linhas estruturais
                                    </label>

                                    <div className="flex items-center justify-end gap-2 rounded-2xl border border-slate-200 px-4">
                                        <Filter className="h-4 w-4 text-slate-400" />
                                        <span className="text-sm text-slate-600">
                                            {result.totalCount.toLocaleString('pt-BR')} registro(s)
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="overflow-hidden">
                            {loadingGrid ? (
                                ...
                                <div className="px-6 py-16 text-center">
                                    <FileSearch className="mx-auto mb-4 h-10 w-10 text-slate-300" />
                                    <h3 className="text-lg font-semibold text-navy">Nenhum registro encontrado</h3>
                                    <p className="mx-auto mt-2 max-w-2xl text-sm text-slate-600">
                                        Ajuste a busca, altere a versão consultada ou reative a visualização das linhas estruturais para ampliar o escopo.
                                    </p>
                                            ...
                                            <TableHeader className="sticky top-0 z-10 bg-slate-50/95 backdrop-blur">
                                                <TableRow className="hover:bg-slate-50/95">
                                                    {[
                                                        ...
                                                        { key: 'full_description', label: 'Descrição completa', sortable: false },
                                                        { key: 'start_date', label: 'Início vigência' },
                                                        { key: 'end_date', label: 'Fim vigência' },
                                                        { key: 'legal_act', label: 'Ato legal' },
                                                        { key: 'legal_number', label: 'Número' },
                                                        { key: 'legal_year', label: 'Ano' },
                                                        ...(currentQuery.includeStructuralRows ? [{ key: 'rowType', label: 'Tipo da linha', sortable: false }] : []),
                                                    ].map((column) => {
                                                        const sortable = column.sortable !== false
                                                        const field = column.key as FiscalNcmSortBy
                                            ...
                                            <TableBody>
                                                {result.items.map((entry) => (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell className="max-w-[380px] whitespace-normal text-slate-600">
                                                            {entry.fullDescription || '-'}
                                                        </TableCell>
                                                        <TableCell>{entry.startDate ? formatDate(entry.startDate) : '-'}</TableCell>
                                                        <TableCell>{entry.endDate ? formatDate(entry.endDate) : '-'}</TableCell>
                                                        <TableCell>{entry.legalAct || '-'}</TableCell>
                                                        <TableCell>{entry.legalNumber || '-'}</TableCell>
                                                        <TableCell>{entry.legalYear || '-'}</TableCell>
                                                        {currentQuery.includeStructuralRows ? (
                                                            <TableCell>
                                                                <Badge
                                                                    variant="outline"
                                                                    className={
                        ...
                        <>
                            <SheetHeader className="border-b bg-white px-6 py-5">
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    >
                                        {selectedEntry.rowType === 'structural' ? 'Linha estrutural' : 'NCM final'}
                                    </Badge>
                                </div>
                                ...
                                <SheetDescription className="mt-1 text-sm text-slate-600">
                                    {selectedEntry.rowType === 'structural'
                                        ? 'Linha de navegação e classificação da tabela oficial. Não deve ser usada como NCM final em perfil tributário.'
                                        : 'Registro final da NCM pronto para consulta normativa e uso em perfil tributário.'}
                                </SheetDescription>
                            </SheetHeader>
                            ...
                            <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
                                ...
                                <section className="rounded-2xl border border-slate-200 bg-white p-5">
                                    <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">Identificação normativa</h3>
                                    <dl className="mt-4 grid gap-4 md:grid-cols-2">
                                        ...
                                        <div>
                                            <dt className="text-xs font-medium text-slate-500">Código de origem</dt>
                                        ...
                                        <div className="md:col-span-2">
                                            <dt className="text-xs font-medium text-slate-500">Descrição completa</dt>
                                            <dd className="mt-1 text-sm text-slate-700">{selectedEntry.fullDescription || 'Não informada nesta linha da versão.'}</dd>
                                        </div>
                                    </dl>
                                </section>

                                <section className="rounded-2xl border border-slate-200 bg-white p-5">
                                    <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">Vigência e ato legal</h3>
                                    <dl className="mt-4 grid gap-4 md:grid-cols-2">
                                        <div>
                                            <dt className="text-xs font-medium text-slate-500">Início de vigência</dt>
                                            <dd className="mt-1 text-sm text-slate-700">{formatDate(selectedEntry.startDate)}</dd>
                                        </div>
                                        <div>
                                            <dt className="text-xs font-medium text-slate-500">Fim de vigência</dt>
                                            <dd className="mt-1 text-sm text-slate-700">{formatDate(selectedEntry.endDate)}</dd>
                                        </div>
                                        <div>
                                            <dt className="text-xs font-medium text-slate-500">Ato legal</dt>
                                            <dd className="mt-1 text-sm text-slate-700">{selectedEntry.legalAct || 'Não informado'}</dd>
                                        </div>
                                        <div>
                                            <dt className="text-xs font-medium text-slate-500">Número / ano</dt>
                                            <dd className="mt-1 text-sm text-slate-700">
                                                {selectedEntry.legalNumber || 'Sem número'}
                                                {selectedEntry.legalYear ? ` / ${selectedEntry.legalYear}` : ''}
                                            </dd>
                                        </div>
                                    </dl>
                                </section>
