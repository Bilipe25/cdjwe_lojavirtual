...
export function EmitterIbscbsStatesSection() {
  ...
  const handleSave = async (data: {
    ...
    toast.success(data.editId ? 'Vínculo de IBS/CBS atualizado.' : 'Vínculo de IBS/CBS adicionado.')
    await reload()
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.error) {
      ...
      toast.success('Vínculo de IBS/CBS removido.')
      await reload()
    }
    setDeletingId(null)
  }
      ...
      <Card className="glass-card border-0">
        <CardHeader className="pb-3">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Cada vínculo do emitente grava base e versão ativa de IBS/CBS, garantindo rastreabilidade e evitando mudanças silenciosas de comportamento fiscal.
          </p>
        </CardHeader>
        <CardContent className="space-y-2">
          {links.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground rounded-xl border border-dashed">
              ...
              <p className="text-sm font-medium">Nenhum vínculo de IBS/CBS configurado.</p>
              <p className="text-xs mt-1">Adicione uma base com versão ativa para abrangência nacional ou por UF.</p>
            </div>
          ) : (
