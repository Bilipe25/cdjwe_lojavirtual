...
export function EmitterIcmsStatesSection() {
    ...
    const handleDelete = async (id: string) => {
        ...
        if (result.error) {
            ...
            toast.success('Vínculo ICMS removido.')
            await reload()
        }
        setDeletingId(null)
    }
    ...
    const handleSave = async (data: { targetUf: string | null; baseId: string; editId?: string }) => {
        ...
        if (result.error) {
            ...
            toast.success(data.editId ? 'Vínculo ICMS atualizado.' : 'Vínculo ICMS adicionado.')
            await reload()
        }
    }
            ...
            <Card className="glass-card border-0">
                <CardHeader className="pb-3">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Esta lista de CSTs por UF será utilizada para calcular o valor do imposto ICMS nos produtos que não tiverem uma configuração específica na UF de destino.
                    </p>
                </CardHeader>
                <CardContent className="space-y-2">
                    {links.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground">
                            ...
                            <p className="text-sm">Nenhum vínculo ICMS configurado.</p>
                            <p className="text-xs mt-1">Clique em &quot;Adicionar&quot; para vincular uma base ICMS.</p>
                        </div>
                    ) : (
                                ...
                                >
                                    <div className="flex items-center gap-3 min-w-0 flex-1">
                                        <div className={`flex items-center justify-center h-8 w-8 rounded-lg shrink-0 ${
                                            link.target_uf ? 'bg-sky-100' : 'bg-emerald-100'
                                        }`}>
                                            {link.target_uf ? (
