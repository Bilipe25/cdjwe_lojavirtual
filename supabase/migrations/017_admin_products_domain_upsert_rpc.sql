-- ============================================================
-- Migration 017: Admin Products Domain Upsert RPC
-- ============================================================
...
BEGIN
    ...
    -- Sync missing variants for this product (product x all colors)
    ...
    -- Apply active/inactive variant map only when explicitly provided.
    IF p_active_variant_ids IS NOT NULL THEN
        UPDATE public.product_variants
           SET is_active = (id = ANY(p_active_variant_ids))
         WHERE product_id = v_product_id;
    END IF;

    -- Apply per-variant price overrides only when explicitly provided.
    IF p_variant_price_overrides IS NOT NULL THEN
        ...
        LOOP
            -- Ignore malformed keys (non-uuid)
            BEGIN
