...
BEGIN
    ...
    IF NULLIF(TRIM(COALESCE(p_national_rule ->> 'cst_code', '')), '') IS NULL THEN
        RAISE EXCEPTION 'National ICMS rule must contain cst_code';
    END IF;
    ...
    ELSE
        UPDATE public.fiscal_icms_rules rule
           SET cst_code = TRIM(COALESCE(p_national_rule ->> 'cst_code', '')),
               ...
               updated_by = v_actor
         WHERE rule.id = v_existing_national_rule_id;
    END IF;

    FOR v_state_item IN SELECT value FROM jsonb_array_elements(COALESCE(p_state_rules, '[]'::JSONB))
    LOOP
        v_state_uf := UPPER(TRIM(COALESCE(v_state_item ->> 'target_uf', '')));

        IF v_state_uf = '' THEN
            RAISE EXCEPTION 'State ICMS rules must contain target_uf';
        END IF;
    ...
    DELETE FROM public.fiscal_icms_rules rule
     WHERE rule.icms_base_id = v_base_id
       ...
       AND NOT (rule.id = ANY(COALESCE(v_keep_state_rule_ids, ARRAY[]::UUID[])));

    IF COALESCE((p_interstate_rule ->> 'enabled')::BOOLEAN, false) THEN
        ...
        SELECT rule.id
         ...
         WHERE rule.icms_base_id = v_base_id
           AND COALESCE(rule.target_uf, '__INTERSTATE__') = COALESCE(v_interstate_target_uf, '__INTERSTATE__')
         ORDER BY rule.created_at ASC
         LIMIT 1;
        ...
        ELSE
            UPDATE public.fiscal_icms_interstate_rules rule
               SET target_uf = v_interstate_target_uf,
                   ...
                   updated_by = v_actor
             WHERE rule.id = v_interstate_rule_id;
        END IF;
    ...
    IF COALESCE((p_st_rule ->> 'st_enabled')::BOOLEAN, false) THEN
        ...
        SELECT rule.id
         ...
         WHERE rule.icms_base_id = v_base_id
           AND COALESCE(rule.target_uf, '__ST__') = COALESCE(v_st_target_uf, '__ST__')
         ORDER BY rule.created_at ASC
         LIMIT 1;
