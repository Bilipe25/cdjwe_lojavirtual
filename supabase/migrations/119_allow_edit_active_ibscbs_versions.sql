...
BEGIN
    ...
    IF v_version_id IS NULL THEN
        ...
        RETURNING version_row.id, version_row.version_number, version_row.status
             INTO v_version_id, v_version_number, v_version_status;
    ...
    ELSE
        UPDATE public.fiscal_ibscbs_base_versions version_row
         ...
         WHERE version_row.id = v_version_id
           ...
           AND version_row.status IN ('draft', 'active')
         RETURNING version_row.version_number, version_row.status
              INTO v_version_number, v_version_status;

        IF v_version_number IS NULL THEN
            RAISE EXCEPTION 'Only draft or active IBS/CBS versions can be edited';
        END IF;
    END IF;
    ...
    SELECT
        ...
        v_created_version,
        v_version_number,
        COALESCE(v_version_status, 'draft')::TEXT;
END;
$$;
